...
export function Breadcrumbs({ activeTab, onNavigate }: BreadcrumbsProps) {
  ...
  const TAB_LABELS: Record<string, { label: string; parent?: string }> = {
    ...
    compras: { label: t('nav.purchases'), parent: t('modules.comercial') },
    ventas: { label: t('nav.sales'), parent: t('modules.comercial') },
    finanzas: { label: t('modules.finance'), parent: t('modules.comercial') },
    costos: { label: t('nav.costs'), parent: t('modules.comercial') },
