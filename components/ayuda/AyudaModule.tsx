...
const CATEGORIA_ICON: Record<DocSection['categoria'], React.ComponentType<{ className?: string }>> = {
  inicio: Rocket,
  nucleo: Box,
  operaciones: Boxes,
  analisis: Brain,
  postventa: ShieldQuestion,
  sistema: Settings,
};
      ...
      <aside className="w-72 border-r border-slate-800 flex flex-col">
        ...
        <nav className="flex-1 overflow-y-auto p-2">
          {CATEGORIA_ORDER.map(cat => {
            ...
            const CatIcon = CATEGORIA_ICON[cat];
            return (
              <div key={cat} className="mb-4">
                <div className="px-2 py-1 text-xs font-medium text-slate-500 uppercase tracking-wider flex items-center gap-1.5">
                  <CatIcon className="w-3.5 h-3.5" />
                  {CATEGORIA_LABEL[cat]}
                </div>
                {items.map(d => {
                  const DocIcon = resolveIcon(d.icon);
                  return (
                    <button
                      key={d.slug}
                    ...
                    >
                      <DocIcon className={`w-4 h-4 mt-0.5 shrink-0 ${slugActivo === d.slug ? 'text-blue-400' : 'text-slate-500'}`} />
