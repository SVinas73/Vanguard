...
export default function BOMManager() {
  ...
  const getEC = (e: BOM['estado']) => ({ borrador: { c: 'bg-slate-800/40 text-slate-300 border-slate-700/40', l: 'Borrador' }, activo: { c: 'bg-slate-800/40 text-slate-300 border-slate-700/40', l: 'Activo' }, obsoleto: { c: 'bg-slate-500/20 text-slate-400 border-slate-500/30', l: 'Obsoleto' }, revision: { c: 'bg-slate-800/40 text-slate-300 border-slate-700/40', l: 'En Revisión' } }[e] || { c: '', l: e });
  const getTC = (t: BOM['tipo']) => ({ produccion: { c: 'text-slate-300', l: 'Producción' }, ingenieria: { c: 'text-slate-300', l: 'Ingeniería' }, venta: { c: 'text-slate-300', l: 'Venta' }, servicio: { c: 'text-slate-300', l: 'Servicio' } }[t] || { c: '', l: t });
  const cs = calcularCostos();

  if (loading) return <div className="flex items-center justify-center p-12"><RefreshCw className="h-8 w-8 animate-spin text-slate-300" /></div>;

  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div><h2 className="text-2xl font-bold text-slate-100 flex items-center gap-3"><Boxes className="h-7 w-7 text-slate-300" />Bill of Materials</h2><p className="text-slate-400 text-sm mt-1">Gestión de listas de materiales</p></div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[{ l: 'Total', v: stats.total, i: Boxes, c: 'blue' }, { l: 'Activos', v: stats.activos, i: CheckCircle, c: 'emerald' }, { l: 'Borradores', v: stats.borradores, i: Edit3, c: 'yellow' }, { l: 'Componentes', v: stats.totalComponentes, i: Package, c: 'purple' }, { l: 'Costo Prom.', v: `$${stats.costoPromedio.toFixed(0)}`, i: DollarSign, c: 'orange' }].map((s, i) => (
          <div key={i} className="bg-slate-900/50 border border-slate-800/50 rounded-xl p-4"><div className="flex items-center gap-2 mb-2"><s.i className={`h-4 w-4 text-slate-200`} /><span className="text-xs text-slate-500">{s.l}</span></div><div className={`text-2xl font-bold text-slate-200`}>{s.v}</div></div>
        ))}
      </div>
      ...
      <div className="bg-slate-900/50 border border-slate-800/50 rounded-xl p-4 flex flex-col md:flex-row gap-4">
        ...
        <div className="flex bg-slate-800/50 rounded-xl p-1">{[{ m: 'table' as const, i: Layers }, { m: 'cards' as const, i: Package }].map(({ m, i: I }) => <button key={m} onClick={() => setViewMode(m)} className={cn('p-2 rounded-lg', viewMode === m ? 'bg-slate-800/40 text-slate-300' : 'text-slate-400')}><I className="h-4 w-4" /></button>)}</div>
      </div>
              ...
              {bomsFiltrados.map(b => { const ec = getEC(b.estado), tc = getTC(b.tipo), exp = expandedRows.has(b.id); return (
                <React.Fragment key={b.id}>
                  <tr className="hover:bg-slate-800/30">
                    ...
                    <td className="px-6 py-4 text-right font-mono text-slate-300">${(b.costoTotal || 0).toLocaleString()}</td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1">
                        <button onClick={() => { setSelectedBOM(b); setModalType('view'); }} className="p-1.5 hover:bg-slate-700 rounded-lg" title="Ver"><Eye className="h-4 w-4 text-slate-300" /></button>
                        <button onClick={() => abrirEditar(b)} className="p-1.5 hover:bg-slate-700 rounded-lg" title="Editar"><Edit3 className="h-4 w-4 text-slate-300" /></button>
                        {b.estado === 'borrador' && <button onClick={() => cambiarEstado(b.id, 'activo')} className="p-1.5 hover:bg-slate-700 rounded-lg" title="Activar"><CheckCircle className="h-4 w-4 text-slate-300" /></button>}
                        ...
                        <button onClick={() => { setCompareBOMs([b, null]); setModalType('compare'); }} className="p-1.5 hover:bg-slate-700 rounded-lg" title="Comparar"><ArrowLeftRight className="h-4 w-4 text-slate-300" /></button>
                        <button onClick={() => duplicar(b)} className="p-1.5 hover:bg-slate-700 rounded-lg" title="Duplicar"><Copy className="h-4 w-4 text-slate-300" /></button>
                        <button onClick={() => eliminar(b.id)} className="p-1.5 hover:bg-slate-700 rounded-lg" title="Eliminar"><Trash2 className="h-4 w-4 text-slate-300" /></button>
                      </div>
                    </td>
                  </tr>
                  {exp && <tr><td colSpan={7} className="px-6 py-4 bg-slate-800/20">
                    <div className="ml-8"><h4 className="text-sm font-semibold text-slate-300 mb-3 flex items-center gap-2"><GitBranch className="h-4 w-4 text-slate-300" />Componentes</h4>
                      <div className="space-y-2">{b.items?.map((it, idx) => (
                        <div key={idx} className="flex items-center justify-between p-3 bg-slate-800/50 rounded-lg">
                          <div className="flex items-center gap-4"><span className="text-xs text-slate-500 w-6">{idx + 1}</span><div><div className="text-sm text-slate-200">{it.componenteDescripcion}</div><div className="text-xs text-slate-500">{it.componenteCodigo}{it.esCritico && <span className="ml-2 text-slate-300">● Crítico</span>}</div></div></div>
                          <div className="flex items-center gap-6"><div className="text-right"><div className="text-sm text-slate-300">{it.cantidad} uds</div><div className="text-xs text-slate-500">${(it.costoUnitario || 0).toFixed(2)} c/u</div></div><div className="font-mono text-slate-300">${(it.costoTotal || 0).toFixed(2)}</div><button onClick={() => { setWhereUsedCodigo(it.componenteCodigo); setModalType('whereUsed'); }} className="p-1.5 hover:bg-slate-700 rounded-lg" title="Donde se usa"><Target className="h-4 w-4 text-slate-300" /></button></div>
                        </div>
                      ))}</div>
                    </div>
                  </td></tr>}
      ...
      {(modalType === 'create' || modalType === 'edit') && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-5xl w-full max-h-[90vh] overflow-y-auto p-6">
            <div className="flex justify-between mb-6"><h3 className="text-xl font-bold text-slate-100 flex items-center gap-2"><Boxes className="h-5 w-5 text-slate-300" />{modalType === 'create' ? 'Nuevo BOM' : 'Editar BOM'}</h3><button onClick={() => setModalType(null)} className="p-2 hover:bg-slate-800 rounded-lg"><X className="h-5 w-5 text-slate-400" /></button></div>
                ...
                <div className="border border-slate-700 rounded-xl max-h-[350px] overflow-y-auto">
                  {!bomItems.length ? <div className="p-8 text-center text-slate-500"><Package className="mx-auto h-10 w-10 mb-2" /><p className="text-sm">Agregue componentes</p></div> : bomItems.map((it, idx) => (
                    <div key={idx} className="p-3 border-b border-slate-700 last:border-0 flex items-center justify-between">
                      <div><div className="text-sm text-slate-200 flex items-center gap-2">{it.componenteDescripcion}{it.esCritico && <AlertTriangle className="h-3 w-3 text-slate-300" />}</div><div className="text-xs text-slate-500">{it.componenteCodigo}</div></div>
                      <div className="flex items-center gap-3"><input type="number" min="1" value={it.cantidad} onChange={e => actualizarItem(idx, { cantidad: parseInt(e.target.value) || 1 })} className="w-16 px-2 py-1 bg-slate-800 border border-slate-700 rounded text-slate-100 text-sm text-center" /><div className="text-right min-w-[60px]"><div className="text-sm font-mono text-slate-300">${(it.costoTotal || 0).toFixed(2)}</div></div><button onClick={() => removerItem(idx)} className="p-1 hover:bg-slate-800/40 rounded"><X className="h-4 w-4 text-slate-300" /></button></div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {modalType === 'compare' && compareBOMs[0] && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-5xl w-full max-h-[90vh] overflow-y-auto p-6">
            <div className="flex justify-between mb-6"><h3 className="text-xl font-bold text-slate-100 flex items-center gap-2"><ArrowLeftRight className="h-5 w-5 text-slate-300" />Comparar Versiones</h3><button onClick={() => setModalType(null)} className="p-2 hover:bg-slate-800 rounded-lg"><X className="h-5 w-5 text-slate-400" /></button></div>
            ...
            {compareBOMs[1] && <>
              ...
              <table className="w-full text-sm border border-slate-700 rounded-xl overflow-hidden"><thead className="bg-slate-800/50"><tr>{['Componente', `v${compareBOMs[0]?.version}`, `v${compareBOMs[1].version}`, 'Cambio'].map(h => <th key={h} className="px-4 py-3 text-left text-slate-400">{h}</th>)}</tr></thead><tbody className="divide-y divide-slate-700">
                {(() => { const all = new Set([...(compareBOMs[0]?.items?.map(i => i.componenteCodigo) || []), ...(compareBOMs[1].items?.map(i => i.componenteCodigo) || [])]); return Array.from(all).map(cod => { const i1 = compareBOMs[0]?.items?.find(i => i.componenteCodigo === cod), i2 = compareBOMs[1]!.items?.find(i => i.componenteCodigo === cod); let cambio = '', cc = ''; if (!i1) { cambio = 'AGREGADO'; cc = 'text-slate-300 bg-slate-800/40'; } else if (!i2) { cambio = 'ELIMINADO'; cc = 'text-slate-300 bg-slate-800/40'; } else if (i1.cantidad !== i2.cantidad) { cambio = `${i2.cantidad > i1.cantidad ? '+' : ''}${i2.cantidad - i1.cantidad}`; cc = 'text-slate-300 bg-slate-800/40'; } else { cambio = '—'; cc = 'text-slate-500'; } return <tr key={cod}><td className="px-4 py-3"><div className="text-slate-200">{i1?.componenteDescripcion || i2?.componenteDescripcion}</div><div className="text-xs text-slate-500">{cod}</div></td><td className="px-4 py-3 text-slate-300">{i1?.cantidad || '—'}</td><td className="px-4 py-3 text-slate-300">{i2?.cantidad || '—'}</td><td className="px-4 py-3"><span className={cn('px-2 py-1 rounded text-xs', cc)}>{cambio}</span></td></tr>; }); })()}
              </tbody></table>
            </>}
            <button onClick={() => setModalType(null)} className="w-full mt-6 px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-xl">Cerrar</button>
          </div>
